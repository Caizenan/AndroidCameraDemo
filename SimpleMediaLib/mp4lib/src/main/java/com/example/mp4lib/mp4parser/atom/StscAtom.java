package com.example.mp4lib.mp4parser.atom;

//
// Source code recreated from a .class file by IntelliJ IDEA
// (powered by FernFlower decompiler)
//


import com.example.mp4lib.mp4parser.annotation.AtomFieldAnnotation;
import com.example.mp4lib.mp4parser.util.ParsableByteArray;

import java.io.IOException;
import java.util.List;

public class StscAtom extends Atom {
    @AtomFieldAnnotation(
            name = "version",
            offset = 8,
            size = 1
    )
    public int version;
    @AtomFieldAnnotation(
            name = "flag",
            size = 3
    )
    public int flag;
    public int entryCount;
    public StscAtom.SampleToChunkItem[] sampleToChunkItems;

    public StscAtom() {
    }

    @Override
    public String getName() {
        return "stsc";
    }

    @Override
    public List<AtomField> getAtomFieldList() {
        List<AtomField> list = super.getAtomFieldList();
        list.addAll(parseAtomFieldFromAnnotations(this, new long[0]));
        long offset = ((AtomField)list.get(list.size() - 1)).getEndPosition();
        list.add(new AtomField("Sample To Chunk Table , size ", this.entryCount, offset, 4L));
        offset += 4L;

        for(int i = 0; i < this.entryCount; ++i) {
            this.sampleToChunkItems[i].addAtomField(list, i, offset);
            offset += 12L;
        }

        return list;
    }

    public void parse(long position, ParsableByteArray input) throws IOException {
        super.parse(position, input);
        this.setOffset(position);
        this.setSize(input.readUnsignedInt());
        this.setType(input.readInt());
        this.version = input.readUnsignedByte();
        this.flag = input.readInt24();
        this.entryCount = input.readInt();
        this.sampleToChunkItems = new StscAtom.SampleToChunkItem[this.entryCount];

        for(int i = 0; i < this.entryCount; ++i) {
            this.sampleToChunkItems[i] = new StscAtom.SampleToChunkItem();
            this.sampleToChunkItems[i].parse(input);
        }

    }

    public StscAtom.SampleToChunkItem[] getSampleToChunkItems() {
        return this.sampleToChunkItems;
    }

    public static class SampleToChunkItem {
        public int firstChunk;
        public int samplesPerChunk;
        public int sampleDescriptionIndex;

        public SampleToChunkItem() {
        }

        void addAtomField(List<AtomField> list, int index, long offset) {
            list.add(new AtomField("" + index, "[firstChunk=" + this.firstChunk + " , samplesPerChunk = " + this.samplesPerChunk + " , sampleDescriptionIndex = " + this.sampleDescriptionIndex + "]", offset, 12L));
        }

        void parse(ParsableByteArray input) {
            this.firstChunk = input.readInt();
            this.samplesPerChunk = input.readInt();
            this.sampleDescriptionIndex = input.readInt();
        }
    }
}
